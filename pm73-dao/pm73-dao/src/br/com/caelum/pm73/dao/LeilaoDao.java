...
public class LeilaoDao {
    ...
    @SuppressWarnings("uporPeriodonchecked")
    ...
    public List<Leilao> listaLeiloesDoUsuario(Usuario usuario) {
        return session.createQuery("select la.leilao " +
                "from Lance la " +
                "where la.usuario = :usuario")
                .setParameter("usuario", usuario)
                .list();
    }

    public double getValorInicialMedioDoUsuario(Usuario usuario) {
        return (Double) session.createQuery("select avg(la.leilao.valorInicial) " +
                "from Lance la " +
                "where la.usuario = :usuario")
                .setParameter("usuario", usuario)
                .uniqueResult();
